...
st.title('Air Quality in Gucheng and Huairou :sparkles:')

st.header('Yearly Weather Data')
...
st.header('Yearly Air Quality Data')
tab1, tab2, tab3, tab4, tab5 = st.tabs(["PM2.5", "PM10", "SO2", "NO2", "CO"])

with tab1:
    average_PM25 = round(yearly_air_quality_df["PM2.5"].mean(), 2)
    st.metric("Average PM2.5 Level", f"{average_PM25} μg/m³")
    max_PM25 = round(yearly_air_quality_df["PM2.5"].max(), 2)
    st.metric("Max PM2.5 Level", f"{max_PM25} μg/m³")
